...
async function handleWallpaperUpload(request, env, ctx) {
  try {
    ...
    // Only accept category values that are actually in our list — free-text
    // like "na" or a typo shouldn't get stored verbatim as a fake category.
    const validCategory = AI_CATEGORY_GROUPS.find(
      (c) => c.toLowerCase() === rawCategoryInput.toLowerCase()
    );
    const rawCategory = validCategory || ""; // treat unrecognized input as "not provided"
    ...
    const messageId = tgResult.result.message_id; // Added: Needed for deletion in cron
